...
let activeFuzzyVal     = 'sugeno'; // Default Sugeno
let currentSetpointVal = 0;
let peakRPM            = 0;
let msgCount           = 0;

let rpmChart, gaugeChart, miniChart;
let cMPage, cSPage, mfMChart, mfSChart;
...
function backToFuzzySelector() {
  document.querySelectorAll('.fuzzy-sub-page').forEach(p => p.style.display = 'none');
  document.getElementById('fuzzy-selector').style.display = 'block';
}

// ==========================================
// CHARTS & MEMBERSHIP FUNCTION
// ==========================================
const chartDefaults = {
  ...
  scales: {
    x: { display: false },
    y: { min: 0, suggestedMax: 300, ticks:{color:'#4a5888', font:{size:10}}, grid:{color:'rgba(80,140,255,0.07)'} }
  }
};
    ...
    rpmChart = new Chart(ctxRpm, {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: 'Setpoint', data: setpointHist, borderColor: '#ef4444', borderDash: [5,5], borderWidth: 1.5, pointRadius: 0, fill: false }
        ]
      },
...
function initMiniChart() {
  ...
  if (ctx) {
    miniChart = new Chart(ctx, {
      type: 'line',
      data: { labels: Array(15).fill(''), datasets: [{ data: miniHistory, borderColor: '#06b6d4', backgroundColor: 'rgba(6,182,212,0.1)', fill: true, tension: 0.4, pointRadius: 0 }] },
      options: chartDefaults
    });
  }
}

// Membership function sesuai Arduino (Domain -100 s.d 100)
...
function makeMFChart(canvasId) {
  ...
  return new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: '#4a5888', font: { size: 10 } } },
        y: { min: 0, max: 1.05, ticks: { color: '#4a5888', font: { size: 10 } } }
      }
    }
  });
}
...
function initFuzzyMamdani() {
  ...
  cMPage = new Chart(ctx, {
    ...
    data: { labels: Array(30).fill(''), datasets: [
      { label:'RPM (Mamdani)', data: mamdaniHist, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.4, pointRadius: 0 },
      { label:'Setpoint', data: setpointHist, borderColor: '#ef4444', borderDash: [5,5], pointRadius: 0, fill: false }
    ]},
    options: chartDefaults
  });
...
function initFuzzySugeno() {
  ...
  cSPage = new Chart(ctx, {
    type: 'line',
    data: { labels: Array(30).fill(''), datasets: [
      { label:'RPM (Sugeno)', data: sugenoHist, borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)', fill: true, tension: 0.4, pointRadius: 0 },
      { label:'Setpoint', data: setpointHist, borderColor: '#ef4444', borderDash: [5,5], pointRadius: 0, fill: false }
    ]},
    options: chartDefaults
  });
...
function confirmSetpoint() {
  ...
  peakRPM = 0; // Reset peak for new overshoot calc
  safeSet('spRPMVal', val);
  safeSet('mSetpoint', val);
  safeSet('activeSetpoint', val + ' RPM');
  
  // Calculate dynamic bar percentage based on max 300 RPM limit
...
function selectLevel(n, btn) {
  ...
  if (mqttClient && mqttClient.connected) {
    // Di ESP32: input 'N' atau 'n' digunakan untuk netral
...
function connectMQTT() {
  ...
  mqttClient.on('message', (topic, message) => {
    ...
    // ESP32 mengirim CSV: "RPM,PWM,ERROR,LEVEL"
    if (raw.includes(',')) {
...
function processIncomingData(rpm, pwm, error, level) {
  ...
  if (gaugeChart) {
    ...
    gaugeChart.update();
    document.getElementById('mRPM').textContent = rpm.toFixed(1);
  }
  ...
  if (activeFuzzyVal === 'mamdani') {
    mamdaniHist.push(rpm); mamdaniHist.shift();
    if (cMPage) cMPage.update();
  } else {
    sugenoHist.push(rpm); sugenoHist.shift();
    if (cSPage) cSPage.update();
  }
...
function logTelemetry(rpm, pwm, err, os, fuzzy, level) {
  ...
  allData.unshift({
    time: timeStr, rpm: rpm.toFixed(1), sp: currentSetpointVal, 
    pwm: pwm.toFixed(0), err: err.toFixed(1), os: os.toFixed(1), 
    type: fuzzy === 'mamdani' ? 'Mamdani' : 'Sugeno', beban: level
  });
...
function updateTelemetryUI() {
  ...
  if (mamData.length > 0) {
    const avgErrM = mamData.reduce((s,d) => s + Math.abs(parseFloat(d.err)), 0) / mamData.length;
    safeSet('statErrM', avgErrM.toFixed(1));
  }

  const sugData = allData.filter(d => d.type === 'Sugeno');
  if (sugData.length > 0) {
    const avgErrS = sugData.reduce((s,d) => s + Math.abs(parseFloat(d.err)), 0) / sugData.length;
    safeSet('statErrS', avgErrS.toFixed(1));
  }
  ...
  if (body) {
    body.innerHTML = allData.slice(0, 50).map((d, i) => `
      <tr>
        <td>${i+1}</td><td>${d.time}</td><td><b>${d.rpm}</b></td><td>${d.sp}</td>
        <td>${d.pwm}</td>
...
function updateFuzzyMetricTables() {
  ...
  levels.forEach(lv => {
    ...
    if(mRows.length && sRows.length) {
      winner = parseFloat(sS.sse) < parseFloat(sM.sse) ? 'Sugeno' : 'Mamdani';
    }
...
window.onload = () => {
  initDashboardCharts();
  selectFuzzyType('sugeno'); // Default ke Sugeno di awal mula
};
